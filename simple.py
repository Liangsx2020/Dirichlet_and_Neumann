...
def main():
    ...
    training_data = 'x_normal'  # type of training data x
    m = 62                      # Krylov subspace dimension for training data
    num_layers = 3              # number of layers in GNP
    embed = 61                  # embedding dimension in GNP
    hidden = 32                 # hidden dimension in MLPs in GNP
    drop_rate = 0.07            # dropout rate in GNP
    ...
    lr = 8.9e-3                   # learning rate in training GNP
    weight_decay = 1e-5          # weight decay in training GNP
